...
class PKI_REST_Users_Controller extends WP_REST_Controller {
	 ...
	 */
	public function activation_user( WP_REST_Request $request ): void {
	 ...
	 * @throws JsonException
	 */
	public function activation_check( WP_REST_Request $request ): void {
		$data = json_decode( $request->get_body(), true, 512, JSON_THROW_ON_ERROR );
	 ...
	 * @return void
	 */
	public function check_user( WP_REST_Request $request ): void {
	 ...
	 * @throws JsonException
	 */
	public function change_user_role( WP_REST_Request $request ): void {
		$data    = json_decode( $request->get_body(), true, 512, JSON_THROW_ON_ERROR );
	 ...
	 * @throws JsonException
	 */
	public function edit_user( WP_REST_Request $request ): void {
		$data    = json_decode( $request->get_body(), true, 512, JSON_THROW_ON_ERROR );
		$user_id = get_option( $data['token'] );
		$user    = get_user_by( 'ID', $user_id );
		...
		if ( isset( $data['form']['passwordRepeat'] ) && ! empty( $data['form']['password'] ) && $data['form']['password'] === $data['form']['passwordRepeat'] ) {
		...
		if ( ! is_wp_error( $user_id ) ) {
			...
			if ( isset( $data['form']['multipleRoles'], $data['form']['roles'] ) && $data['form']['multipleRoles'] && count( $data['form']['roles'] ) > 1 ) {
	 ...
	 * @throws JsonException
	 */
	public function get_role_statistics( WP_REST_Request $request ): void {
		$data    = json_decode( $request->get_body(), true, 512, JSON_THROW_ON_ERROR );
		$user_id = get_option( $data['token'] );
	 ...
	 * @return void
	 */
	public function logout_user( WP_REST_Request $request ): void {
		$token = $request->get_body();
	 ...
	 * @throws JsonException
	 */
	public function resend_activation_link( WP_REST_Request $request ): void {
		$data    = json_decode( $request->get_body(), true, 512, JSON_THROW_ON_ERROR );
		$user_id = get_option( $data['token'] );
		$user    = get_user_by( 'ID', $user_id );
	 ...
	 * @throws JsonException
	 */
	public function reset_password( WP_REST_Request $request ): void {
		$data = json_decode( $request->get_body(), true, 512, JSON_THROW_ON_ERROR );

		if ( ! empty( $data['login'] ) && ! empty( $data['token'] ) && ! empty( $data['password'] ) && ! empty( $data['passwordRepeat'] )
	 ...
	 * @throws JsonException
	 */
	public function reset_password_request( WP_REST_Request $request ): void {
		$data = json_decode( $request->get_body(), true, 512, JSON_THROW_ON_ERROR );

		if ( ! empty( $data['login'] ) && validate_username( $data['login'] ) ) {
			$user_id = is_email( $data['login'] ) ? email_exists( $data['login'] ) : username_exists( $data['login'] );
